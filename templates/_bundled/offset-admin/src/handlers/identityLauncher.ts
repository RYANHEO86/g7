/**
 * IDV Modal Launcher (offset-admin)
 *
...
const logger = ((window as any).G7Core?.createLogger?.('Template:offset-admin:IdentityLauncher')) ?? {
  log: (...args: unknown[]) => console.log('[Template:offset-admin:IdentityLauncher]', ...args),
  warn: (...args: unknown[]) => console.warn('[Template:offset-admin:IdentityLauncher]', ...args),
  error: (...args: unknown[]) => console.error('[Template:offset-admin:IdentityLauncher]', ...args),
};
...
export function registerSirsoftAdminBasicIdentityLauncher(): void {
  ...
  logger.log('IDV launcher registered (offset-admin)');
}
