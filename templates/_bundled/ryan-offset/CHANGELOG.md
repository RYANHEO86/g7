...
- 메인 페이지(home.json) 를 오프셋 테마 디자인으로 전면 교체 — 9개 섹션(Hero 자동 슬라이더 / Quick 적용업종 / Themes 라인업 / 알림마당+POPUP / Research 연구사업 / Glance AICT / SNS CTA / Footer)
- 메인 페이지 전용 composite 컴포넌트 13개 신규 추가 (`src/components/composite/`): `HeroCarousel`, `PopupSlider`, `NoticeTab`, `OverlayHeader`, `RevealOnScroll`, `CountUp`, `QuickSection`, `ThemesSection`, `NoticeArea`, `ResearchSection`, `GlanceSection`, `SnsSection`, `AictFooter`
- `home-aict` 스코프 CSS 시스템 추가 (`src/styles/home-aict/`) — `_tokens.css`(라이트/다크 디자인 토큰), `_fonts.css`(Paperlogy+Pretendard CDN), `foundation.css` 및 8개 섹션 CSS 가 `.aict-home` 부모 selector 로 격리되어 다른 페이지 영향 없음
- `_user_base.json` 의 글로벌 헤더/푸터에 조건부 가드(`if: "{{!_global.hideDefaultHeader}}"`/`hideDefaultFooter`) 추가 — 메인 페이지는 OverlayHeader + AictFooter 만 표시
- 자산: `assets/images/aict/**` 약 58개 이미지 추가 (브랜드 로고/Hero 슬라이드/Themes/Research/News/Footer 마크/Popup 포스터 등)

### Changed

- 기존 `home.json` (게시판 통계+최신글 중심 232줄 구조)을 백업(`home.json.bak`) 후 오프셋 테마 메인으로 교체
