...
const logger = ((window as any).G7Core?.createLogger?.('Template:ryan-offset')) ?? {
    log: (...args: unknown[]) => console.log('[Template:ryan-offset]', ...args),
    warn: (...args: unknown[]) => console.warn('[Template:ryan-offset]', ...args),
    error: (...args: unknown[]) => console.error('[Template:ryan-offset]', ...args),
};
...
import { registerRyanOffsetIdentityLauncher } from './handlers/identityLauncher';
  ...
  if (typeof window !== 'undefined') {
    ...
    const registerHandlers = () => {
      ...
      if (actionDispatcher) {
        ...
        registerRyanOffsetIdentityLauncher();
      } else {
        retryCount++;
